...
function Form({ addService }: FormProps): ReactElement {
  ...
  function convertToService(newService: {
    service: string;
    login: string;
    password: string;
    url: string;
    ...
    return {
      handleservice: newService.service,
      handlelogin: newService.login,
      handlepassword: newService.password,
      handleurl: newService.url,
    };
  }
  ...
  const register = () => {
    ...
    if (formValidation()) {
      ...
      const serviceToAdd = convertToService(newService);
